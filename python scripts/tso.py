...
if '__main__' == __name__:
    ...
    # interfaces = ['h1-eth0', 'h1-eth1', 'h2-eth0', 'r1-eth0', 'r1-eth1', 'r1-eth2']
    # for iface in interfaces:
    #     for host in [h1, h2, r1]:
    #         if iface.startswith(host.name):
    #             host.cmd(f"ethtool -K {iface} tso off")

    # Execute the commands within the xterm windows
    r1.cmd('bash -c "sleep 1; xterm -e \'tcpdump -i r1-eth2 -w te.pcap\' &"')
    #h1.cmd('bash -c "sleep 1; xterm -e \'tcpdump -i h1-eth1 -w tem.pcap\' &"')
    # h2.cmd('bash -c "sleep 1; xterm -e \'tcpdump -i h2-eth0 -w servside2.pcap\' &"')
    h2.cmd('bash -c "sleep 1; xterm -e \'iperf -s -i 1\' &"')
    h1.cmd('bash -c "sleep 1; xterm -e \'iperf -c 10.0.2.2 -n 100M -i 1\' &"')
    # h1.cmd('bash -c "sleep 1; xterm -e \'iperf -c 10.0.2.2 -n 200M -i 1 > day4_en.txt\' &"')
    # h1.cmd('bash -c "sleep 1; xterm -e \'iperf -c 10.0.2.2 -t 10 -n 500M ' &"')
