...
max_len = max(len(throughput_scenario1), len(throughput_scenario2), len(throughput_scenario3))
...
plt.plot(df['Time (s)'], df['Scenario 3 Throughput (Mbits/sec)'], label='MPTCP same MTU')
plt.xlabel('Time (s)')
...
plt.boxplot([df['Scenario 1 Throughput (Mbits/sec)'].dropna(), 
            ...
            labels=['TCP', 'MPTCP different MTU', 'MPTCP same MTU'])
plt.ylabel('Throughput (Mbits/sec)')
